...
- **Logo, trois déclinaisons, chacune à sa place** (décision du 11/09/2026) : en en-tête desktop le **nom seul** (`LogoTexte.tsx`, version étroite de l'ancien site, sans dessin ni baseline) ; en en-tête mobile le **pictogramme seul** (`Marque.tsx`, les trois foudres — le nom seul ferait 290 px de large) ; au pied de page le **logo complet** en blanc, dessin + nom + baseline. Jamais le logo complet en en-tête : la baseline y est illisible.
